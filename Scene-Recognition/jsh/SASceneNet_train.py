    ...
    with torch.no_grad():
        for i, (mini_batch) in enumerate(dataloader):
            ...
            if USE_CUDA:
                ...
                gray_segmentation_tensor=process_segmentation_output(semantic_scores, gray_images_tensor, batch_size=len(gray_images_np), threshold=0.5)
...
def process_segmentation_output(segmentation_output, gray_images, threshold=0.5, batch_size):
    ...
    return final_output
"""
def process_segmentation_output_test(segmentation_output, gray_images, batch_size,threshold=0.5):
    # 확률 임계값보다 높은 부분만 남기고, 나머지는 0으로 설정
    thresholded_output = torch.where(segmentation_output > threshold, segmentation_output, torch.tensor(0.0).cuda())

    # 흑백 이미지에 채널 차원 추가 (10, 1, 224, 224)
    # segmentation_output의 차원에 맞게 gray_images_with_channel 차원 조정
    gray_images_with_channel = gray_images.expand(batch_size, 151, 224, 224)
