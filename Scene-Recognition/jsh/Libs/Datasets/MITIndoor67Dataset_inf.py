...
class MITIndoor67Dataset(Dataset):
    ...
    def __init__(self, root_dir, set, tencrops=False, SemRGB=True):
        ...
        if not self.TenCrop:
            self.test_transforms_img = transforms.Compose([
            ...
            if not SemRGB:
                self.test_transforms_sem = transforms.Compose([
                ...
                self.test_transforms_scores = transforms.Compose([
                    transforms.Resize(self.resizeSize),
                    transforms.CenterCrop(self.outputSize),
                    transforms.ToTensor(),
                ])
    ...
    def __getitem__(self, idx):
        ...
        # Load semantic segmentation scores -> 여기에 semantic segmatation을 불러와서, 넣는다.
        ...
        if self.set == "train":
            ...
            img = self.test_transforms_img(img)
            sem = self.test_transforms_sem(sem)
            semScore = self.test_transforms_scores(semScore)
