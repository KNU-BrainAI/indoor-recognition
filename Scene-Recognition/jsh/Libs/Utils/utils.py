...
def make_one_hot(labels, semantic_scores, C=151):
    ...
    # ?
    # 왜 C+1을 할까
    # 왜 C= 151일까, object info는 150
    if len(labels.shape) == 3:
        labels = torch.unsqueeze(labels, 1) # (10,1,224,224)

    # Semantic Labels
    one_hot = torch.cuda.FloatTensor(labels.size(0), C + 1, labels.size(2), labels.size(3)).zero_()
    # 10, 152, 224, 224 all 0 -> one hot encoding을 위한 작업.

    for n in range(labels.size(1)):# 0 to 151 까지
