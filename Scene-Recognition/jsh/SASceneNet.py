...
class SASceneNet(nn.Module):
    ...
    def __init__(self, arch, scene_classes, semantic_classes=151):
        ...
        arch :                  the transfer learning architecture, RESNET-50;
        scene_classes :         67 (MIT_INDOORSET)
        semantic_classes=151 :  
        """
        ...
        ) # indices(입력텐서의 최대값의 인덱스를 반환)리턴

        self.encoder1 = base.layer1 #resnet의 first layer set. cnn layer pooling layer로 구성
        self.encoder2 = base.layer2 #resnet의 second layer set. 
        self.encoder3 = base.layer3 #resnet의 third layer set
        self.encoder4 = base.layer4 #resnet의 fourth layer set
    ...
    def forward(self, x, sem):
        ...
        #           RGB Branch            #
        # ------------------------------- #
        x, pool_indices = self.in_block(x) # indices(입력텐서의 최대값의 인덱스를 반환)리턴
        e1 = self.encoder1(x)
        e2 = self.encoder2(e1)
        e3 = self.encoder3(e2)
        e4 = self.encoder4(e3)
