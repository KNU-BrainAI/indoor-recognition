...
def expand_grayscale_to_3ch(img):
    ...
    expanded_img = Image.fromarray(img_array)
...
class Detector():
    ...
    def detect_one(self, **kwargs):
        ...
        # image read at 3ch
        img = kwargs.pop('pil_img', None) or cv2.imread(kwargs['img_path'])
        img = expand_grayscale_to_3ch(img)
        # img resize 480 x 480
        img = img.resize((480, 480))
        detections = self._predict_pil(img, **kwargs)

        if kwargs.get('return_img', False):
            np_img = np.array(img)
            visualization.draw_dt_on_np(np_img, detections, print_dt=True, **kwargs)
            return np_img
