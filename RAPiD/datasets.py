...
class Dataset4YoloAngle(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # augmentation
        if self.enable_aug:
            img, labels = self.augment_PIL(img, labels[:gt_num])

        # pad to square
        img, labels, pad_info = rect_to_square(img, labels,
        ...
        labels = normalize_bbox(labels, self.img_size, self.img_size)
        ...
        # print(labels.shape, origin_labels.shape)
        print(len(labels), len(categories), len(str(img_id)), len(pad_info))
        return img, labels, categories, str(img_id), pad_info

    def augment_PIL(self, img, labels):
        if np.random.rand() > 0.5:
            img, labels = augUtils.random_shift(img, labels, (0.2, 0.8))
