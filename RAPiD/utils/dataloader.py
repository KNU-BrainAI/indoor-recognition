...
class Images4Detector():
    ...
    def __next__(self):
        if self.i >= len(self.img_names):
            raise StopIteration

        img_name = self.img_names[self.i]
        # load frame
        img_path = os.path.join(self.img_dir, img_name)
        frame_pil = cv2.imread(img_path)
        
        # Convert the PIL image to a NumPy array
        frame_np = np.array(frame_pil)
        
        # Ensure that the image is grayscale (1-channel)
        if len(frame_np.shape) == 2:
            # Expand the 1-channel image to 3-channel (RGB)
        ...
        self.i += 1  # Increment the counter for the next iteration
